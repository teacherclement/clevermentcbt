...
// ============================================================
// ASSESSMENT CODE AUTO-FORMATTING (XXX-XXX-XXX)
// ============================================================
function formatAssessmentCodeValue(value) {
    if (!value) return '';
    var cleaned = value.replace(/[^A-Za-z0-9]/g, '').toUpperCase().slice(0, 9);
    var formatted = '';
...
function setupAssessmentCodeInput() {
    var input = document.getElementById('assessmentCode');
    if (!input) return;
    input.setAttribute('maxlength', '11');
    input.setAttribute('autocomplete', 'off');
    input.setAttribute('placeholder', 'e.g., MAT-PRI-865');
    
    input.addEventListener('input', function(e) {
        var start = e.target.selectionStart;
        var oldLen = e.target.value.length;
        var formatted = formatAssessmentCodeValue(e.target.value);
        e.target.value = formatted;
        var newLen = formatted.length;
        // Adjust cursor: if we added a hyphen, move forward
        try {
            var diff = newLen - oldLen;
            var newPos = start + diff;
            // Handle backspace over hyphen: if user deleted hyphen, stay
    ...
    input.addEventListener('paste', function(e) {
        ...
        var formatted = formatAssessmentCodeValue(pasted);
...
function updateURL(page) {
    try {
        ...
        if (currentAssessmentCode) {
            params.set('code', currentAssessmentCode);
        } else {
            // keep existing code param if present and we are in student flow
            var existingCode = params.get('code');
            if (existingCode && (page === 'student-assessment' || page === 'student' || page === 'results' || page === 'certificate')) {
                // keep it
            } else if (page === 'landing' || page === 'teacher' || page === 'admin') {
                params.delete('code');
            }
        }
        var newURL = window.location.pathname + '?' + params.toString();
        window.history.pushState({ page: page, code: currentAssessmentCode }, '', newURL);
        localStorage.setItem(LS_KEYS.currentPage, page);
        localStorage.setItem('cleverment_page', page);
        // Also save to legacy key for compatibility
        localStorage.setItem('cleverment_current_page', page);
    } catch(e) {
        var fallbackURL = window.location.pathname + '?page=' + page + (currentAssessmentCode ? '&code=' + encodeURIComponent(currentAssessmentCode) : '');
        window.history.pushState({ page: page }, '', fallbackURL);
    }
}
...
function showPageFromURL(page) {
    ...
    if (page === 'landing' || page === '' || !page) {
        document.getElementById('landingPage').style.display = 'block';
        localStorage.setItem(LS_KEYS.currentPage, 'landing');
    } else if (page === 'student') {
        document.getElementById('studentAccess').style.display = 'block';
        // Restore code input if we have saved code
        var codeInput = document.getElementById('assessmentCode');
        if (codeInput && currentAssessmentCode) {
            codeInput.value = formatAssessmentCodeValue(currentAssessmentCode);
        } else if (codeInput) {
            var savedCode = localStorage.getItem(LS_KEYS.currentCode);
            if (savedCode) codeInput.value = formatAssessmentCodeValue(savedCode);
        }
    } else if (page === 'student-assessment') {
        document.getElementById('studentAssessmentView').style.display = 'block';
        // If we have studentQuestions, we are in quiz, else show info form
        if (studentQuestions && studentQuestions.length > 0 && document.getElementById('studentQuizSection')) {
            // Will be handled by restore logic, default to info form if no quiz
            var isQuizVisible = localStorage.getItem(LS_KEYS.studentState);
            if (isQuizVisible) {
                // Let restore handle, but show view at least
                document.getElementById('studentAssessmentView').style.display = 'block';
            } else {
        ...
        if (typeof showTeacherLoginForm === 'function') showTeacherLoginForm();
    } else if (page === 'teacher-dashboard') {
        document.getElementById('teacherDashboard').style.display = 'block';
        if (currentTeacher) {
            var nameEl = document.getElementById('teacherDashboardName');
            var emailEl = document.getElementById('teacherDashboardEmail');
            if (nameEl) nameEl.textContent = 'Welcome, ' + currentTeacher.name + '!';
            if (emailEl) emailEl.textContent = currentTeacher.email;
            if (typeof renderTeacherDashboard === 'function') renderTeacherDashboard();
            if (typeof renderCSVHistory === 'function') renderCSVHistory();
        }
    } else if (page === 'admin') {
...
// ============================================================
// PERSISTENT STATE MANAGEMENT (Fix refresh issue)
// ============================================================
var LS_KEYS = {
  currentPage: 'cleverment_page_v2',
  currentAssessment: 'cleverment_current_assessment',
  currentCode: 'cleverment_current_code',
  teacherSession: 'cleverment_teacher_session',
  studentState: 'cleverment_student_state',
  studentResult: 'cleverment_student_result',
  adminSession: 'cleverment_admin_session'
};

function saveCurrentAssessmentToStorage() {
    try {
        if (currentAssessment) {
            localStorage.setItem(LS_KEYS.currentAssessment, JSON.stringify(currentAssessment));
            localStorage.setItem(LS_KEYS.currentCode, currentAssessmentCode);
        }
    } catch(e) {}
}
...
function saveTeacherSession() {
    try {
        if (currentTeacher) {
            localStorage.setItem(LS_KEYS.teacherSession, JSON.stringify(currentTeacher));
        } else {
            localStorage.removeItem(LS_KEYS.teacherSession);
        }
    } catch(e) {}
}

function saveStudentQuizState() {
    try {
        var state = {
            ...
            quizStarted: document.getElementById('studentQuizSection') && document.getElementById('studentQuizSection').style.display === 'block',
            infoFormVisible: document.getElementById('studentInfoForm') && document.getElementById('studentInfoForm').style.display === 'block',
            timestamp: Date.now()
        };
        localStorage.setItem(LS_KEYS.studentState, JSON.stringify(state));
    } catch(e) { console.log('save student state error', e); }
}

function clearStudentQuizState() {
    try { localStorage.removeItem(LS_KEYS.studentState); } catch(e) {}
}

function saveStudentResultState() {
    try {
        var state = {
            studentName: studentName,
            studentClass: studentClass,
            ...
            studentTimeTaken: studentTimeTaken,
            currentAssessmentCode: currentAssessmentCode,
            timestamp: Date.now()
        };
        localStorage.setItem(LS_KEYS.studentResult, JSON.stringify(state));
    } catch(e) {}
}
...
function showStudentAccess() {
    ...
    document.getElementById('studentAccess').style.display = 'block';
    var acInput = document.getElementById('assessmentCode');
    if (acInput) { acInput.value = ''; setTimeout(function(){ acInput.focus(); }, 100); }
    setupAssessmentCodeInput();
    updateURL('student');
}
    ...
    getTeacherFromDatabase(email).then(function(teacher) {
        if (!teacher) {
            ...
            if (found) {
                ...
                try { saveTeacherSession(); } catch(e) {}
...
function teacherLogout() {
    ...
    try { 
        localStorage.removeItem(LS_KEYS.teacherSession);
        localStorage.removeItem(LS_KEYS.currentPage);
        localStorage.removeItem('cleverment_page');
    } catch(e) {}
    document.getElementById('teacherDashboard').style.display = 'none';
    document.getElementById('teacherAuth').style.display = 'block';
    if (typeof showTeacherLoginForm === 'function') showTeacherLoginForm();
    var pwd = document.getElementById('teacherLoginPassword');
    if (pwd) pwd.value = '';
    updateURL('landing');
}
...
function verifyAssessmentCode() {
    ...
    getAssessmentByCodeFromDatabase(code).then(function(found) {
        if (found) {
            ...
            try { saveCurrentAssessmentToStorage(); } catch(e) {}
...
function startStudentQuiz() {
    ...
    studentStartTimer();
    try { saveStudentQuizState(); saveCurrentAssessmentToStorage(); } catch(e) {}
    updateURL('student-assessment');
}
...
function studentGoToQuestion(index) {
    if (index >= 0 && index < studentQuestions.length) {
        ...
        try { saveStudentQuizState(); } catch(e) {}
    }
}
...
function studentStartTimer() {
    ...
    if (studentTimeLimit === 0) {
        var disp = document.getElementById('studentTimerDisplay');
        if (disp) disp.textContent = '∞';
        return;
    }
    // Show initial time immediately
    var initialDisplay = document.getElementById('studentTimerDisplay');
    if (initialDisplay) {
        var initMins = Math.floor(studentTimeRemaining / 60);
        var initSecs = studentTimeRemaining % 60;
        initialDisplay.textContent = String(initMins).padStart(2, '0') + ':' + String(initSecs).padStart(2, '0');
    }
    studentTimerInterval = setInterval(function() {
        ...
        // Auto-save timer and answers every second
        try { saveStudentQuizState(); } catch(e) {}
...
function studentSubmitQuiz() {
    ...
    try { saveStudentResultState(); } catch(e) {}
    // Keep studentState for results page but will clear after certificate or new assessment
    // clearStudentQuizState(); // Don't clear immediately so refresh on results keeps data
...
function adminLogin() {
    ...
    if (password === ADMIN_PASSWORD) {
        try { localStorage.setItem(LS_KEYS.adminSession, 'logged_in'); } catch(e) {}
...
// ============================================================
// SAVE PAGE STATE & RESTORE ON REFRESH (New robust implementation)
// ============================================================
...
window.addEventListener('beforeunload', function() {
    try {
        ...
        else if (document.getElementById('studentResultsSection').style.display === 'block') currentPage = 'results';
        else if (document.getElementById('studentCertificateSection').style.display === 'block') currentPage = 'certificate';
        
        if (currentPage) {
            localStorage.setItem('cleverment_page', currentPage);
            localStorage.setItem(LS_KEYS.currentPage, currentPage.replace('studentAccess','student').replace('studentAssessmentView','student-assessment').replace('teacherAuth','teacher').replace('adminAuth','admin').replace('teacherDashboard','teacher-dashboard').replace('adminDashboard','admin-dashboard'));
        }
        // Save student state if in quiz
        if (studentQuestions && studentQuestions.length > 0) {
            saveStudentQuizState();
        }
        if (currentAssessment) saveCurrentAssessmentToStorage();
        if (currentTeacher) saveTeacherSession();
    } catch(e) {}
});

function restoreFullStateOnLoad() {
    try {
        // Restore teacher session
        var teacherSaved = localStorage.getItem(LS_KEYS.teacherSession);
        if (teacherSaved) {
            try { currentTeacher = JSON.parse(teacherSaved); } catch(e) {}
        }
        // Restore assessment
        var assessSaved = localStorage.getItem(LS_KEYS.currentAssessment);
        var codeSaved = localStorage.getItem(LS_KEYS.currentCode);
        if (assessSaved) {
            try {
                currentAssessment = JSON.parse(assessSaved);
                currentAssessmentCode = codeSaved || currentAssessment.code || '';
            } catch(e) {}
        }
        ...
        if (codeFromURL) {
            var formattedFromURL = formatAssessmentCodeValue(codeFromURL);
            currentAssessmentCode = formattedFromURL;
            var input = document.getElementById('assessmentCode');
            if (input) input.value = formattedFromURL;
        } else if (codeSaved) {
            currentAssessmentCode = codeSaved;
        }

        // Try to restore student state
        var studentState = null;
        try {
            var ss = localStorage.getItem(LS_KEYS.studentState);
            if (ss) studentState = JSON.parse(ss);
        } catch(e) {}

        var adminSession = localStorage.getItem(LS_KEYS.adminSession);
        var targetPage = pageFromURL || localStorage.getItem(LS_KEYS.currentPage) || localStorage.getItem('cleverment_page') || localStorage.getItem('cleverment_current_page') || 'landing';
        // If admin session exists and no explicit page, restore admin dashboard
        if (adminSession && !pageFromURL && targetPage === 'landing') {
            var savedAdminPage = localStorage.getItem(LS_KEYS.currentPage);
            if (savedAdminPage === 'admin-dashboard' || savedAdminPage === 'admin') {
                targetPage = 'admin-dashboard';
            }
        }
        // Normalize legacy values
        if (targetPage === 'studentAccess') targetPage = 'student';
        if (targetPage === 'studentAssessmentView') targetPage = 'student-assessment';
        if (targetPage === 'teacherAuth') targetPage = 'teacher';
        if (targetPage === 'adminAuth') targetPage = 'admin';
        if (targetPage === 'teacherDashboard') targetPage = 'teacher-dashboard';
        if (targetPage === 'adminDashboard') targetPage = 'admin-dashboard';

        if (studentState && studentState.studentQuestions && studentState.studentQuestions.length > 0) {
            studentName = studentState.studentName || '';
            studentClass = studentState.studentClass || '';
            studentSubject = studentState.studentSubject || '';
            studentQuestions = studentState.studentQuestions || [];
            studentAnswers = studentState.studentAnswers || [];
            studentCurrentIndex = studentState.studentCurrentIndex || 0;
            studentTimeRemaining = studentState.studentTimeRemaining || 0;
            studentTimeLimit = studentState.studentTimeLimit || 0;
            studentIsTimeUp = studentState.studentIsTimeUp || false;
            if (studentState.currentAssessment) {
                currentAssessment = studentState.currentAssessment;
                currentAssessmentCode = studentState.currentAssessmentCode || currentAssessmentCode;
            }

            if (studentState.quizStarted && targetPage === 'student-assessment') {
                // Restore quiz UI
                var allSections = ['landingPage', 'studentAccess', 'studentAssessmentView', 'teacherAuth', 'teacherDashboard', 'adminAuth', 'adminDashboard', 'studentCertificateSection', 'studentResultsSection'];
                for (var i = 0; i < allSections.length; i++) {
                    var el = document.getElementById(allSections[i]);
                    if (el) el.style.display = 'none';
                }
                document.getElementById('studentAssessmentView').style.display = 'block';
                document.getElementById('studentInfoForm').style.display = 'none';
                var quizSec = document.getElementById('studentQuizSection');
                if (quizSec) quizSec.style.display = 'block';
                var titleEl = document.getElementById('studentQuizTitle');
                var dispEl = document.getElementById('studentQuizDisplay');
                if (titleEl) titleEl.textContent = studentSubject || (currentAssessment ? currentAssessment.subject : 'Assessment');
                if (dispEl) dispEl.textContent = 'Student: ' + studentName + ' | Class: ' + studentClass;
                if (typeof studentCreateQuestionBoxes === 'function') studentCreateQuestionBoxes();
                if (typeof studentDisplayQuestion === 'function') studentDisplayQuestion();
                if (typeof studentUpdateNavigationButtons === 'function') studentUpdateNavigationButtons();
                if (studentTimeLimit > 0 && studentTimeRemaining > 0) {
                    if (typeof studentStartTimer === 'function') studentStartTimer();
                } else if (studentTimeLimit === 0) {
                    var timerDisp = document.getElementById('studentTimerDisplay');
                    if (timerDisp) timerDisp.textContent = '∞';
                }
                return;
            } else if (studentState.infoFormVisible && currentAssessment) {
                var secs = ['landingPage', 'studentAccess', 'studentAssessmentView', 'teacherAuth', 'teacherDashboard', 'adminAuth', 'adminDashboard', 'studentCertificateSection', 'studentResultsSection'];
                for (var j = 0; j < secs.length; j++) {
                    var e2 = document.getElementById(secs[j]);
                    if (e2) e2.style.display = 'none';
                }
                document.getElementById('studentAssessmentView').style.display = 'block';
                document.getElementById('studentInfoForm').style.display = 'block';
                var tEl = document.getElementById('studentAssessmentTitle');
                var sEl = document.getElementById('studentAssessmentSubject');
                if (tEl) tEl.textContent = 'Assessment: ' + currentAssessment.subject;
                if (sEl) sEl.textContent = 'Class: ' + currentAssessment.className + ' | ' + currentAssessment.questions.length + ' questions';
                var nameIn = document.getElementById('studentNameInput');
                if (nameIn) nameIn.value = studentName;
                return;
            }
        }

        // Standard routing
        if (targetPage && targetPage !== 'landing') {
            showPageFromURL(targetPage);
            // Extra for teacher dashboard
            if (targetPage === 'teacher-dashboard' && currentTeacher) {
                var nEl = document.getElementById('teacherDashboardName');
                var emEl = document.getElementById('teacherDashboardEmail');
                if (nEl) nEl.textContent = 'Welcome, ' + currentTeacher.name + '!';
                if (emEl) emEl.textContent = currentTeacher.email;
            }
            // If student page with code but no assessment loaded, try to verify automatically
            if (targetPage === 'student' && codeFromURL && !currentAssessment) {
                setTimeout(function(){ 
                    try { verifyAssessmentCode(); } catch(e) {}
                }, 800);
            }
        } else {
            showPageFromURL('landing');
        }
    } catch(e) {
...
window.addEventListener('load', function() {
    setupAssessmentCodeInput();
    restoreFullStateOnLoad();
});

window.addEventListener('popstate', function(event) {
    var page = getPageFromURL() || (event.state && event.state.page) || 'landing';
    showPageFromURL(page);
});

// ============================================================
// REGISTER SERVICE WORKER WITH AUTO-UPDATE (Fix cache issue)
// ============================================================

if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/sw.js?v=2.5', { updateViaCache: 'none' })
        .then(function(registration) {
            console.log('Service Worker registered successfully!');
...
// Also setup code input on DOMContentLoaded for faster response
document.addEventListener('DOMContentLoaded', function() {
    setupAssessmentCodeInput();
});
